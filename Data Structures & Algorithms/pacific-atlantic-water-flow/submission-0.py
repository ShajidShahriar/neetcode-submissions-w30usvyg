...
class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        # add the rows and colum toucghin the pacific ocean first 
        for j in range(n): #top row 
            p_que.append((0 , j))
            p_seen.add((0 , j))
        
        for i in range(1, m): #left column
            p_que.append((i , 0))
            p_seen.add((i , 0))

        # add the rows and columns touching the atlantic ocean 

        for i in range( m - 1 ): # right column 
            a_que.append((i , n - 1))
            a_seen.add((i , n - 1 ))
